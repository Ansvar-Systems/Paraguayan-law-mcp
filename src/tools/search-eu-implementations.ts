/**
 * search_eu_implementations — Search EU directives/regulations that have Paraguayan implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_paraguayan_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  paraguayan_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as paraguayan_statute_count
  ...
  if (input.has_paraguayan_implementation) {
    sql += ' HAVING paraguayan_statute_count > 0';
  }
