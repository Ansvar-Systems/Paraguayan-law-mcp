#!/usr/bin/env tsx
/**
 * Paraguayan Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://bacn.gov.py';
const PORTAL_NAME = 'Paraguay Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/paraguayan-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Paraguayan Law MCP — Data Freshness Check');
