#!/usr/bin/env tsx
/**
 * Paraguay Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from bacn.gov.py (Akoma Ntoso HTML).
 *
 * Features:
 ...
 *
 * Data source: bacn.gov.py (National Council for Law Reporting)
...
import { parseParaguayLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Paraguay Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('====================================================\n');
  console.log(`  Source: bacn.gov.py (National Council for Law Reporting)`);
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseParaguayLawHtml(html, act);
  ...
  console.log(`\n  Source:      bacn.gov.py (Akoma Ntoso HTML)`);
