#!/usr/bin/env tsx
/**
 * Drift detection for Paraguay Law MCP.
 *
 * Checks if upstream Paraguay Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Paraguay Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'paraguayan-law-mcp/1.0 drift-detect' },
      });
