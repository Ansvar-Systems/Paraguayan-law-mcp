#!/usr/bin/env tsx
/**
 * Database builder for Paraguay Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Paraguay Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Paraguayan statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'PY');
    insertMeta.run('source', 'bacn.gov.py');
